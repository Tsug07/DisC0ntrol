...
class Dashboard(ctk.CTk):
    ...
    def __init__(self, config_path: str):
        ...
        # Initial scan
        self.after(200, self._refresh_bots)

        # Cleanup orphan PIDs on startup, then auto-start bots if enabled
    ...
    def _build_ui(self):
        ...
        ctk.CTkButton(
            ...
            width=90,
            **btn_style,
        ).grid(row=0, column=5, padx=(4, 16), pady=10)
        ...
        self._statusbar.grid(row=2, column=0, sticky="ew", padx=12, pady=(0, 4))
    ...
    def _refresh_bots(self):
        """Re-scan directories and rebuild bot cards."""
        ...
        # Scan
        self._bots = self.scanner.scan_all()
    ...
    def _build_card_grid(self):
        """Create bot cards in a responsive grid layout."""
        # Calculate columns based on window width
        cols = max(1, self._scroll_frame.winfo_width() // CARD_MIN_WIDTH)
        if cols < 1:
            cols = 2  # Default before first render

        for col in range(cols):
            self._scroll_frame.grid_columnconfigure(col, weight=1)
    ...
    def _startup_sequence(self):
        """Clean orphans, then auto-start bots if enabled."""
        if self._bots:
